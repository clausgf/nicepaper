...
from extensions.epaper.util import humanize_datetime_age
...
def device_config_card(paths: EpaperPaths, project_name: str, device_name: str, image_base_url: str) -> None:
    ...
    @ui.refreshable
    def _body() -> None:
        """Panel type/Screen selects plus the mismatch hint, refreshable so
        the hint (which depends on panel_type_id) redraws right after a
        ...
        if binding.screen_id and binding.screen_id not in all_screen_names:
            ui.label(f'Assigned screen "{binding.screen_id}" no longer exists.') \
    ...
    with ui.card().tight().classes('w-full'):
        with ui.card_section().props('dense').classes('w-full'):
            ...
            with ui.row().classes('items-center w-full gap-1'):
                ui.space()
                ui.label(f'as of {humanize_datetime_age(reported_at, datetime.datetime.now(datetime.timezone.utc))}') \
