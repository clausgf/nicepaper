...
def quantize(rgb_image: Image.Image, color_model: ColorModel) -> Image.Image:
...
class ImageCache:
    ...
    async def put_data(self, rgb_image: Image.Image, last_update_at: datetime.datetime,
                       ...
                       color_model: Optional[ColorModel] = None) -> ImageMetadata:
        ...
        if color_model:
            ...
            served_image = await asyncio.to_thread(
                self._quantize_and_save, rgb_image, color_model, palette_filename)

        version = await asyncio.to_thread(self._version, served_image, color_model)
    ...
    @staticmethod
    def _version(image: Image.Image, color_model: Optional[ColorModel]) -> str:
        ...
        if color_model:
            h.update(repr(color_model.palette).encode())
        return h.hexdigest()


    async def get_image_path(self, color_model: Optional[ColorModel] = None) -> Optional[str]:
        """Path of the cached PNG for this palette, or of the RGB image
        when `color_model` is None. Quantizes on demand if the palette
    ...
    def _quantize_and_save(self, rgb_image: Image.Image, color_model: ColorModel,
                           palette_filename: str) -> Image.Image:
        palette_image = quantize(rgb_image, color_model)
        palette_image.save(palette_filename, format="PNG", bits=8, compress_level=9)
        return palette_image


    async def get_image(self, color_model: Optional[ColorModel] = None) -> Optional[Image.Image]:
